...
fetch(api2URL)
  ...
  .then(data => {
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      liEl.classList.add('event-item'); // Add class to the li element

      let eventName = document.createElement('h3');
      eventName.id = `event-name-${i}`;
      eventName.classList.add('event-name'); // Add class to the h3 element
      eventName.textContent = events[i].name;

      let dateEl = document.createElement('p');
      dateEl.id = `event-date-${i}`;
      dateEl.classList.add('event-date'); // Add class to the p element
      let date = new Date(events[i].dates.start.dateTime);
      dateEl.textContent = date.toLocaleString();

      let venue = document.createElement('p');
      venue.id = `event-venue-${i}`;
      venue.classList.add('event-venue'); // Add class to the p element
      venue.textContent = events[i]._embedded.venues[0].name;

      let image = document.createElement('img');
      image.id = `event-image-${i}`;
      image.classList.add('event-image'); // Add class to the img element
      image.src = events[i].images[0].url;

      let btn = document.createElement('button');
      btn.id = `buy-button-${i}`;
      btn.classList.add('buy-button'); // Add class to the button element
      ...
      btnAddToFavorites.id = `favorites-button-${i}`;
      btnAddToFavorites.classList.add('favorites-button'); // Add class to the button element
